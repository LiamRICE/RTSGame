...
func calculate_unit_transform() -> void:
	...
	# TODO - if sharp turn : slow based on turn radius sharp turn
	# TODO - if end : brake early to stop on target
	...
	# fetch the rotation for climb angle
	var facing_vertical = global_transform.origin
	facing_vertical.y = facing.y
	# set the quaternion to the correct vertical direction intantly
	var vertical_rotation = Quaternion(Basis.looking_at(facing_vertical)).normalized()
	# saves the current rotation quaternion then looks at the target and sets the target quaternion 2D flat
	var spin_rotation = Quaternion(Basis.looking_at(facing)).normalized()
	
	# Rotates the object towards the vertical facing of the unit
	#self.quaternion = vertical_rotation
	# Rotates the object by spherical interpolation towards the spin facing the unit is targeting
	self.quaternion = rotate_towards(self.quaternion, spin_rotation, ROTATION_SPEED)
